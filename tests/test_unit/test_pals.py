...
def test_signals():
    # pylint: disable=bad-continuation, comparison-with-callable
...
def test_get_rpc():
    ...
    assert rpc == {"jsonrpc": "2.0", "method": "test"}
    rpc = json.loads(pals.get_rpc("test", foo="bar"))
    assert rpc == {"jsonrpc": "2.0", "method": "test", "params": {"foo": "bar"}}
    rpc = json.loads(pals.get_rpc("test", "1234"))
    assert rpc == {"jsonrpc": "2.0", "method": "test", "id": "1234"}
...
def test_send_rpc():
    ...
    assert json.loads(sock.send_queue[0]) == start
    assert json.loads(sock.send_queue[1]) == stream
...
def test_handle_rpc():
    ...
    assert json.loads(sock.send_queue[0]) == start_rpc
    ...
    assert json.loads(sock.send_queue[1]) == procinfo_rpc
...
def test_forward_stdin():
    ...
    assert json.loads(sock.send_queue[0]) == expected

    expected = {"jsonrpc": "2.0", "method": "stdin", "params": {"eof": True}}
    assert json.loads(sock.send_queue[1]) == expected
