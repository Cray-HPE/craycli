...
def parse_hostfile(hostfile):
    """ Parse a nid list from a host file """
    nids = []
    for line in hostfile:
        match = re.match(r"\s*nid(\d+)", line)
        if match:
            nids.append(int(match.group(1), 10))
...
def get_hostlist(node_list, node_list_file, exclude_node_list, exclude_node_list_file):
    ...
    if node_list:
        nidlist = parse_rangelist(node_list)
    elif node_list_file:
        for line in node_list_file:
            line = line.strip()
            if line and line[0] != "#":
                nidlist.extend(parse_rangelist(line))
    elif "PBS_NODEFILE" in os.environ:
        with open(os.environ["PBS_NODEFILE"]) as nodefile:
            nidlist = parse_hostfile(nodefile)

    # Build exclude node list from command line arguments
    if exclude_node_list:
        excllist = parse_rangelist(exclude_node_list)
    elif exclude_node_list_file:
        for line in exclude_node_list_file:
            line = line.strip()
            if line and line[0] != "#":
                excllist.extend(parse_rangelist(line))

    # Remove excluded nodes from nid list
    nidlist = [nid for nid in nidlist if nid not in excllist]

    # Convert nids into hostnames
    hostlist = ["nid%06d" % nid for nid in nidlist]
...
def get_exclusive(access_mode):
    ...
    if not access_mode:
        return None
    elif access_mode[0].lower() == "e":
        return True
    elif access_mode[0].lower() == "s":
        return False
...
def parse_mpmd(executable, args, pes, wdir):
    ...
    cmds = [dict(argv=argv, nranks=pes, umask=umask, wdir=wdir)]
...
@core.option(
    ...
    multiple=True,
    help="set an application environment variable",
)
...
def cli(
    ...
    * ALPS_APP_PE - Rank ID
    ...
    launchreq = {
        "cmds": parse_mpmd(executable, args, pes, get_wdir(wdir)),
        "hosts": get_hostlist(
    ...
    app = PALSApp()
    exit_codes = app.launch(launchreq, not bypass_app_transfer, False, procinfo_file)
